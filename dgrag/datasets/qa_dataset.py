...
from dgrag.datasets.kg_dataset import (
    KGDataset,
    atomic_torch_save,
    build_stage2_fingerprint,
)
from dgrag.text_emb_models import BaseTextEmbModel
from dgrag.utils import get_rank
from dgrag.utils.qa_utils import entities_to_mask
...
class QADataset(InMemoryDataset):
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                "Processed QA cache for %s is unreadable at %s: %s. Rebuilding.",
                self.name,
    ...
    def process(self) -> None:
        ...
        atomic_torch_save(ent2doc, os.path.join(self.processed_dir, "ent2doc.pt"))
        ...
        for num_sample in num_samples:
            ...
            splits.append(split)
            offset += num_sample
        atomic_torch_save(splits, self.processed_paths[0])
