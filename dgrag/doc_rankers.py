...
class IDFWeightedScoreTopKRanker(BaseDocRanker):
    """
    Rank documents from top-k entity predictions while preserving entity scores.

    This keeps the noise control of top-k entity filtering, applies IDF to avoid
    over-crediting entities that appear in many documents, and retains the
    retriever's score magnitude instead of replacing selected entities by IDF
    alone.
    """
    ...
    def __call__(self, ent_pred: torch.Tensor) -> torch.Tensor:
        top_k = min(self.top_k, ent_pred.shape[-1])
        top_k_ent_pred = torch.topk(ent_pred, top_k, dim=-1)
        idf_weight = torch.gather(
            self.idf_weight.expand(ent_pred.shape[0], -1), 1, top_k_ent_pred.indices
        )
        weighted_score = top_k_ent_pred.values * idf_weight
        masked_ent_pred = torch.zeros_like(ent_pred, device=ent_pred.device)
        masked_ent_pred.scatter_(1, top_k_ent_pred.indices, weighted_score)
        doc_pred = torch.sparse.mm(masked_ent_pred, self.ent2doc)
        return doc_pred
