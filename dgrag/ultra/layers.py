...
from dgrag.ultra import variadic
...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(
        ...
        if edge_parallel_shard_strategy != "contiguous":
            raise ValueError(
                "edge_parallel_shard_strategy currently supports only 'contiguous'"
            )
    ...
    def forward(
        ...
        if self._edge_parallel_enabled():
            return self._edge_parallel_forward(
                input,
                relation,
                boundary,
                edge_index,
    ...
    def _edge_parallel_enabled(self):
        return (
    ...
    def _edge_slice(self, edge_index, edge_type, edge_weight):
        world_size = dist.get_world_size()
        rank = dist.get_rank()
        num_edges = edge_type.numel()
        start = (num_edges * rank) // world_size
        end = (num_edges * (rank + 1)) // world_size
        return edge_index[:, start:end], edge_type[start:end], edge_weight[start:end]

    def _edge_parallel_forward(
        self, input, relation, boundary, edge_index, edge_type, size, edge_weight
    ):
        if self.aggregate_func not in {"sum", "mean"}:
            raise ValueError(
                "edge-parallel GeneralizedRelationalConv currently supports only "
                "'sum' or 'mean' aggregation"
            )
        if self.message_func not in {"transe", "distmult"}:
            raise ValueError(
                "edge-parallel GeneralizedRelationalConv currently supports only "
                "'transe' or 'distmult' messages"
            )

        local_edge_index, local_edge_type, local_edge_weight = self._edge_slice(
            edge_index, edge_type, edge_weight
        )
        num_nodes = size[1] if size is not None else input.size(1)
        local_update = input.new_zeros(input.size(0), num_nodes, self.input_dim)

        if local_edge_type.numel() > 0:
            src, dst = local_edge_index
            input_src = input[:, src, :]
            relation_edge = relation[:, local_edge_type, :]
            if self.message_func == "transe":
                message = input_src + relation_edge
            else:
                message = input_src * relation_edge

            message = message * local_edge_weight.view(1, -1, 1)
            scatter_index = dst.view(1, -1, 1).expand(-1, -1, message.size(-1))
            scatter_index = scatter_index.expand(message.size(0), -1, -1)
            local_update.scatter_add_(1, scatter_index, message)

        global_update = differentiable_all_reduce(
            local_update, op=dist.ReduceOp.SUM
        )
        update = global_update + boundary

        if self.aggregate_func == "mean":
            local_degree = input.new_zeros(num_nodes)
            if local_edge_type.numel() > 0:
                local_degree.scatter_add_(
                    0,
                    local_edge_index[1],
                    input.new_ones(local_edge_type.numel()),
                )
            global_degree = differentiable_all_reduce(
                local_degree, op=dist.ReduceOp.SUM
            )
            degree_out = (global_degree + 1).clamp_min(1).view(1, -1, 1)
            update = update / degree_out

        return self.update(update, input)
