...
def native_scatter_softmax(src, index, dim=-1, eps=1e-12, dim_size=None):
    """
    ...
    if src.numel() == 0:
        return src
    # Node IDs index groups, not positions in the edge tensor. After removing
    # easy edges, the largest node ID can exceed the number of remaining edges.
    group_shape = list(src.shape)
    group_shape[dim] = (
        max(src.size(dim), int(index.max()) + 1) if dim_size is None else dim_size
    )
    max_value_per_index = src.new_zeros(group_shape)
    ...
    sum_per_index = src.new_zeros(group_shape)
