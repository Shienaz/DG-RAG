...
def evaluate_absolute_retrieval(target, ks=(5, 10, 20)):
    """
    Evaluate absolute top-k retrieval coverage from unfiltered target rankings.

    `target` is the second tuple returned by `batch_evaluate`: the absolute
    zero-based ranks of all gold items and the number of gold items per query.
    Unlike `evaluate`, this does not apply filtered ranking, so it matches the
    documents that can actually enter a stage3 top-k prompt.
    """
    answer_ranking, num_target = target
    metric = {}
    if len(num_target) == 0:
        for k in ks:
            metric[f"support_recall@{int(k)}"] = 0.0
            metric[f"all_support_hit@{int(k)}"] = 0.0
        return metric

    valid_mask = num_target > 0
    for k in ks:
        k = int(k)
        answer_score = (answer_ranking < k).float()
        hit_count = variadic.variadic_sum(answer_score, num_target)
        support_recall = hit_count / num_target.clamp_min(1).float()
        all_support_hit = hit_count == num_target.float()
        if valid_mask.any():
            support_recall = support_recall[valid_mask]
            all_support_hit = all_support_hit[valid_mask]
        metric[f"support_recall@{k}"] = support_recall.mean().item()
        metric[f"all_support_hit@{k}"] = all_support_hit.float().mean().item()
