...
from dgrag import GFMRetriever
from dgrag.evaluation import RetrievalEvaluator
from dgrag.llms import BaseLanguageModel
...
def agent_reasoning(
    ...
    current_query = query
    thoughts: list[str] = []
    retrieved_docs = retriever.retrieve(current_query, top_k=cfg.test.top_k)
    logs = []
    while step <= cfg.test.max_steps:
        ...
        new_ret_docs = retriever.retrieve(response, top_k=cfg.test.top_k)
...
def main(cfg: DictConfig) -> None:
    ...
    test_data = retriever.qa_data.raw_test_data
    max_samples = (
        ...
        for i in tqdm(range(max_samples)):
            ...
            if sample["id"] in processed_data:
                ...
                result = agent_reasoning(
                    cfg, retriever, llm, agent_prompt_builder, query
                )
